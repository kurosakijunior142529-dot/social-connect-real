...
import { Smile, CornerUpLeft, Pencil, Trash2, Languages, MoreHorizontal } from "lucide-react";
...
export function MessageActions({
  message, ctx, mine, onReply, onEdit, onDelete, onTranslated,
}: {
  message: { id: string; content: string | null; sender_id: string };
  ctx: MessageContext;
  mine: boolean;
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-64 p-2 glass border-white/10" align="end">
        <div className="flex gap-1 pb-2 border-b border-white/5 justify-around">
          {QUICK_EMOJIS.map((e) => (
            <button key={e} onClick={() => react(e)} className="h-8 w-8 grid place-items-center rounded-full hover:bg-white/10 text-lg">
              {e}
            </button>
          ))}
        </div>
        <div className="pt-2 flex flex-col text-sm">
          <button onClick={() => { onReply(message); setOpen(false); }} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/5">
            <CornerUpLeft className="h-4 w-4" /> Responder
          </button>
          <button onClick={translate} disabled={ai.busy} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/5 disabled:opacity-50">
            <Languages className="h-4 w-4" /> Traduzir
          </button>
          {mine ? (
            <>
              <button onClick={() => { onEdit(message); setOpen(false); }} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/5">
                <Pencil className="h-4 w-4" /> Editar
              </button>
              <button onClick={() => { onDelete(message.id); setOpen(false); }} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/5 text-destructive">
