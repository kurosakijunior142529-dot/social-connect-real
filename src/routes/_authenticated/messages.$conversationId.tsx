  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5rem)] md:h-[calc(100vh-4rem)] md:rounded-2xl md:overflow-hidden md:bg-[color:var(--surface)]">
      <header className="flex items-center gap-2 px-3 h-14 glass-heavy hairline-b sticky top-0 z-10">
        <Link to="/messages" className="p-2 -ml-1 rounded-full active:bg-[color:var(--surface-2)] md:hidden" aria-label="Voltar"><ArrowLeft className="h-5 w-5" strokeWidth={1.8} /></Link>
        {other ? (
          <>
            <UserAvatar avatarPath={other.avatar_url} displayName={other.display_name} className="h-9 w-9" />
            <Link to="/u/$username" params={{ username: other.username }} className="flex-1 min-w-0 leading-tight">
              <div className="font-semibold text-[15px] truncate">{other.display_name}</div>
              <div className="text-[11px] text-muted-foreground truncate">@{other.username}</div>
            </Link>
            ...
            <button onClick={() => startCall({ id: other.id, username: other.username, display_name: other.display_name, avatar_url: other.avatar_url }, "audio")}
              disabled={isBlockedPair} className="p-2 rounded-full active:bg-[color:var(--surface-2)] disabled:opacity-40" aria-label="Voz"><Phone className="h-[18px] w-[18px]" strokeWidth={1.8} /></button>
            <button onClick={() => startCall({ id: other.id, username: other.username, display_name: other.display_name, avatar_url: other.avatar_url }, "video")}
              disabled={isBlockedPair} className="p-2 rounded-full active:bg-[color:var(--surface-2)] disabled:opacity-40" aria-label="Vídeo"><Video className="h-[18px] w-[18px]" strokeWidth={1.8} /></button>
